--REGISTROS DE CONTROL PARA COMPROBAR SI REGISTRO YA EXISTE O NO
--LA EXCEPCION EN ESTE CASO NO SE UTILIZA PARA MANEJO DE ERRORES
--SE UTILIZA PARA PODER INSERTAR UN VALOR EN LA TABLA
SET SERVEROUTPUT ON
DECLARE
  REG REGIONS%ROWTYPE;
  REG_CONTROL REGIONS.REGION_ID%TYPE;
BEGIN
   REG.REGION_ID:=100;
   REG.REGION_NAME:='AFRICA';
   SELECT REGION_ID INTO REG_CONTROL FROM REGIONS
   WHERE REGION_ID=REG.REGION_ID;
   DBMS_OUTPUT.PUT_LINE('LA REGION YA EXISTE');
EXCEPTION   
   WHEN NO_DATA_FOUND  THEN
        INSERT INTO REGIONS VALUES (REG.REGION_ID,REG.REGION_NAME);
        COMMIT;
END;